...
namespace Vegetarian.Application.Implements.Auth
{
    public class AuthService : IAuthService
    {
        ...
        private readonly IHangfireService _backgroundJobService;

        public AuthService(UserManager<User> userManager, ITokenService tokenService, IUnitOfWork unitOfWork, IHangfireService backgroundJobService)
        {
            Env.Load();
        ...
        private void SendEmail(Guid userId, string email, string otp)
        {
            ...
            _backgroundJobService.Enqueue<IEmailService>(x => x.EmailSender(email, "Một email đã gửi đến email của bạn . Hãy nhập mã xác nhận", htmlBody));
        }
